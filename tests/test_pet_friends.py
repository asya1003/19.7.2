...
def test_add_pet_photo_incorret_type(pet_photo='images/123.png'):
    """Проверяем что можно нельзя добавить фото питомца в другом формате кроме jpeg/image  """
...
def test_add_new_pet_without_photo_with_invalid_data(name='Барбоскин', animal_type='двортерьер',
                                     age= 'опо'):
    ...
    if age.isdigit():
        ...
        print('AttributeError, возраст должен быть указан цифрой')

def test_unsuccessful_delete_self_pet():
    """Проверяем возможность удаления питомца с id которого нет"""

    # Получаем ключ auth_key
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")

    # Берём id  питомца не из списка и отправляем запрос на удаление
    pet_id = '5512'
    if pet_id in my_pets.values():
        ...
        print('Error: Нет питомца с таким id')
